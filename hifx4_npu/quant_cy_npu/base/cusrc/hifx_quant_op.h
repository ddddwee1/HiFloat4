...
public: 
    ...
    aifunc void Compute(int mn){
        ...
        vand((__ubuf__ uint16_t*)signbuf.ptr(), (__ubuf__ uint16_t*)inp_tsr.ptr(), signmaskbuf2.ptr(), BATCH/64, 1, 1, 0, 8, 8, 0);
        pipe_barrier(PIPE_V);
        vor((__ubuf__ uint16_t*)signbuf.ptr(), (__ubuf__ uint16_t*)signbuf.ptr(), signmaskbuf1.ptr(), BATCH/64, 1, 1, 0, 8, 8, 0);
        ...
        pipe_barrier(PIPE_V);
        vmul(out_tsr.ptr(), out_tsr.ptr(), signbuf.ptr(), BATCH/64, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        
        // copy_ubuf_to_ubuf(out_tsr.vptr(), mantbuf.vptr(), 0, 1, BATCH/8, 0, 0);
        // pipe_barrier(PIPE_V);
